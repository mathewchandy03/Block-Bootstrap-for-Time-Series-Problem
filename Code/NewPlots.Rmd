```{r}
graph_bts <- function(cov, phi, target, width, nrep = 1000, sizes = c(100, 200, 300, 400, 500), types = c('estCI', 'mbtCI', 'altCI', 'bcaCIest', 'bcaCImbt', 'bcaCIalt'), parameters = c('mu', 'sigma', 'phi'), level = .95) {
...
phi <- 0
target <- c(0, 1, phi)
cov <- c(mychk(sim_0_1h, target), mychk(sim_0_2h, target), mychk(sim_0_3h, target), mychk(sim_0_4h, target), mychk(sim_0_5h, target))
...
phi <- .2
target <- c(0, 1, phi)
cov <- c(mychk(sim_.2_1h, target), mychk(sim_.2_2h, target), mychk(sim_.2_3h, target), mychk(sim_.2_4h, target), mychk(sim_.2_5h, target))
graph_bts(cov, phi, target, width = 25)
...
phi <- .4
target <- c(0, 1, phi)
cov <- c(mychk(sim_.4_1h, target), mychk(sim_.4_2h, target), mychk(sim_.4_3h, target), mychk(sim_.4_4h, target), mychk(sim_.4_5h, target))
