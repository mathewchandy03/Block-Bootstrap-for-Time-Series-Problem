...
exact CI of the 
coverage rate,\citep{clopper1934use}
which is an estimate of a proportion with $R = 10,000$. We used the
R \textsl{PropCIs} package to achieve this.\citep{PropCIs}
The choice of
Clopper-Pearson was motivated by the Wald 
interval's poor coverage as the proportion approaches 0 or 1, as noted by 
\citet{brown2001interval}, although when we tried Wald intervals, the coverage
rate intervals
did not appear to have large differences. 
If the 
proportion 0.95 
...
effective sample size is increased. \citep{geyer2011introduction} Additionally, 
...
\jy{Are we reporting the C-P intervals? Clarify the desciption and
  cite the reference.}
...
The
plot and results for $l = \lceil 2n^{1/3} \rceil$ are included in the Appendix.
Performance generally seems to be better for $l = \lceil n^{1/3} \rceil$, but
importantly, the changes in performance when varying other parameters do not
change when a different block size is used.
...
recentered percentile CI $\phi$ coverage rates were greater for stronger 
dependences 
regardless of the sign, $\rho$ coverage rates for these CIs seem to be greater 
only for 
stronger negative dependencies and lesser for stronger positive dependences. 
\mc{What could be an explanation for this?}
\jy{I don't follow this. Are you talking about mu only? Even for mu,
  the statement does not look right.} \mc{for lag-1 autocorrelation}
\jy{This would invalidate our explanation using the effective sample
  size. Let's tune it down or remove it.}
However, the other 
trends 
and patterns
...
short.
\jy{Inconsistent means not consistent and consistency is a desired
  property of an estimator. When we talk about width of a CI, what
  does consistency mean? Do an throughout check, please.}
\mc{I was basically trying to say that the widths of the CI vary more 
for BC CIs than for percentile or recentered CIs, I think it may be simpler to
just say that they are shorter}
...
marginal
\jy{Recall that t is not different from normal when sample size is
  large. So the recommendation is to do t always since it performs
...
\jy{Don't discuss these. The recommended method is regardless of what
  the dependence struture and marginald istribution are. That's why we
  did the study. Just gave recommendations based on both
  scenarios. Rewrite this paragraph.}
the dependence structure or the marginal distribution is already known, a 
lesser $n$ may be 
...
\jy{These statements are not to the point. Let's not discuss the case
  where phi is know or not. They are not known. We want the method to
  be robust to phi and the true marginal distribution.}
...
The results for $\sigma_x$ with $l = \lceil 2n^{1/3} \rceil$ look very similar 
to 
...
\jy{Put the small paragraphs into a bigger one.}

As shown in \textbf{Figure~\ref{fig:norm2}},
