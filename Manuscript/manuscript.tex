...
\jy{I'd slightly increase the heights of the figures such that two of them
  would occupied a whole page. This reduces the complexity of floating in
  finding best locations.}
\mc{addressed}

\jy{Clarify normal margins in captions.}
\mc{addressed}
...
distribution is discussed in the Appendix. Overall, the results with an 
underlying marginal exponential distribution are very similar to the results 
with an underlying normal distribution discussed above.
...
3) $\phi$, the lag-1 autocorrelation coefficient of $W_t$.
Let $\rho$ be the lag-1 autocorrelation coefficient of $X_t$.
Although we may have set $\rho$ to these five levels for $X_t$: 
...
\jy{see demo code in \texttt{copula.R}}


To calibrate the copula parameters, we first constructed a multivariate
exponential distribution from a normal copula with 
$\rho \in \{-0.4, -0.2, 0.0, 0.2, 0.4\} $using the \textsl{copula} package.
\citep{hofert2018elements} Then we computed $E[XY]$, which 
requires double integration using the \textsl{pracma} package. \citep{pracma} 
Finally, we obtained true $\phi = E[XY] - 1$, which was used as the target
parameter when evaluating CI performance when estimating $\phi$.
After this step, we again plotted empirical coverage 
rates for $\mu$, $\sigma_w$, and $\phi$, as well as 95\% confidence intervals of 
...
\jy{Cite my book on the copula package}
\mc{addressed}

\jy{Let's call this process marginally transformed AR(1) proess. Itself is not
  an AR(1) process. Correct the captions.}
\mc{addressed}
...
\begin{figure}[tbp]
  ...
  \caption{Empirical coverage rates of different 95\% block bootstrap CIs for
    the marginal mean $\mu$ of a marginally transformed AR(1) process with an 
    underlying exponential distribution ($\lambda = 1$) with AR 
    coefficient
    $\phi \in \{-0.4, 0.2, 0, 0.2, 0.4\}$ and series length
    $n \in \{100, 200, 400, 800, 1600, 3200\}$ with
...
\begin{figure}[tbp]
  ...
  \caption{Empirical coverage rates of different 95\% block bootstrap CIs for
    the marginal standard deviation $\sigma_x$ of a marginally transformed AR(1) 
    process with an 
    underlying exponential distribution ($\lambda = 1$) with AR 
    coefficient $\phi \in \{-0.4, 0.2, 0, 0.2, 0.4\}$ and series length 
...
\begin{figure}[tbp]
  ...
  \caption{Empirical coverage rates of different 95\% block bootstrap CIs for 
    the first-order autocorrelation coefficient $\phi$ of a marginally 
    transformed AR(1) process with 
    an 
    underlying exponential distribution ($\lambda = 1$) with
    $\phi \in \{-0.4, 0.2, 0, 0.2, 0.4\}$ and series length
    $n \in \{100, 200, 400, 800, 1600, 3200\}$ transformed to
    an exponential distribution with $\lambda = 1$ based on 10,000 replicates. 
    The error bars represent 95\% CIs of the real coverage rates.}
...
\begin{figure}[tbp]
  \centering
  \includegraphics[width=\textwidth]{figures/exp_pho_intervals}
  \caption{50 replicate percentile, BC, and recentered percentile CIs for 
...
It seems that range of sample sizes needed 
...
still deteriorate in coverage of $\phi$ as sample size increases. As exhibited 
...
\jy{comment on that the recentered percentile CI for autocorrelation still works
  as the point estimator is unbiased.}
