...
\author{
Mathew Chandy*$^{a}$, Elizabeth Schifano$^{a}$, Jun Yan$^{a}$ \\ \medskip 
...
\jy{Are the results from exponential margins the same? If so, we can put them in
  an appendix; make sure we have the data generation process accurately
  described. Then briefly summarize and refer to it at the end of the simulation
  section.} \mc{addressed}


\jy{The 80-width is for maximum. No need to re-wrap shorter lines because it
  would show a lot of diff while there is no diff.}
\mc{addressed}
...
originated by \citet{efron1979bootstrap} - but for serially dependent data.
\mc{style guide says to have citations after punctuation, not sure what to do 
with textual citations}
Early ideas of block
bootstrap were developed not long after the standard 
...
$j = 1, \ldots, n - l + 1$. Now we draw $k$ blocks from the $(n - l + 1)$ blocks 
...
be the upper $\alpha$ quantile of the standard normal distribution. 
...
upper $\alpha$ quantile of a $t$ distribution with $k$ degrees of freedom. With 
block bootstrapping, 
...
$b \in \{1, \ldots, B\}$. Define $\alpha_1 = \Phi(2\hat{z}_0 - z_{\alpha/2})$ 
and $\alpha_2 = \Phi(2\hat{z}_0 - z_{1 - \alpha/2})$. 
...
CIs for each parameter as described in the last section. We can estimate $\mu$, 
...
\jy{The fonts of labels/annotatios in these figures are too big compared to the
  captions. Try pointsize = 9. The plots also seem oversized. Generate them in R
  with the right size in mind.} \mc{addressed}
...
\begin{figure}[tbp]
  ...
  \caption{Empirical coverage rates of different 95\% block bootstrap CIs for
    the marginal standard deviation $\sigma_x$ of an AR(1) process with AR 
...
other methods contain 95\%. The standard, percentile, BC, and BCA, and 
...
issue for $\sigma_x$ when $\phi = -0.4$. Standard, percentile, BC, BCA, and 
...
\textbf{Figure~\ref{fig:phi}} suggests that while standard, Student's $t$, and 
...
standard, Student's $t$, and recentered percentile CIs should be considered as 
...
slightly as the absolute value increases for standard, Student's $t$, and 
recentered percentile CIs. For the values of $\phi$ observed, there are no 
examples of over-coverage for standard, Student's $t$, BC, BCA, or recentered 
...
\jy{Could do one for the exponential margin study too. 
Maybe for the negative dependence.}
\mc{created negative dependence plots not included in manuscript yet}
...
estimate $\hat\theta_n$ is asymptotically unbiased. Since the width is based on 
...
An additional simulation study in which block bootstrap is used to estimate 
parameters of an AR(1) process with an underlying exponential marginal 
distribution is discussed in the appendix.
...
respect to estimating $\phi$ only for standard, Student's $t$, and recentered 
...
\section*{Simulation Study with Exponential Marginal Distribution}

\jy{Motivation; data generation (calibration of copula parameters to give the
  desired rho); simulation design; plots; brief discussion of results.}
\mc{addressed}
...
AR(1) process with an underlying exponential marginal distribution. The 
motivation for this additional simulation study was to observe if the results of 
the main simulation study will change if the underlying marginal distribution is
not normal. 


We generated time series $X_t$ from a 1st order autoregressive 
(AR(1)) process as we did previously, but transformed the data from a standard
normal distribution to an exponential distribution with rate $\lambda = 1$, so 
that for $t \in \{1, \ldots, n\}$, we can define:
\begin{equation}
W_t = F^{-1}(\Phi(X_t), 1)
\end{equation}
where $F^{-1}(p, \lambda)$ is the quantile function for the exponential 
distribution with rate parameter $\lambda$. 


The simulation design was the same, except the target parameters of $W_t$ were:
1) $\mu = 1$, the mean of $W_t$;
2) $\sigma_w = 1$, the standard deviation of $W_t$; and
3) $\rho$, the lag-1 autocorrelation coefficient of $W_t$
Although we may have set $\phi$ to these five levels for $X_t$: 
$\{-0.4, -0.2, 0.0, 0.2, 0.4\}$, because of the transformation,
the new five true autocorrelation coefficients for $W_t$ were different.
\mc{I may need some help explaining the calibration of copula parameters}
To calibrate the copula parameters, we first constructed a multivariate
exponential distribution from a normal copula using the \textsl{copula} package.
\citep{copula} Then we computed $E[XY]$, which 
requires double integration using the \textsl{pracma} package. \citep{pracma} 
Finally, we obtained true $\rho = E[XY] - 1$, which was used as the target
parameter when evaluating CI performance when estimating $\rho$.
After this step, we again plotted empirical coverage 
rates for $\mu$, $\sigma$, and $\rho$, as well as 95\% confidence intervals of 
the real coverage. These are displayed in 
\textbf{Figures~\ref{fig:exp_mu}--\ref{fig:exp_rho}}. 
...
\begin{figure}[tbp]
  ...
  \caption{Empirical coverage rates of different 95\% block bootstrap CIs for
    the marginal mean $\mu$ of an AR(1) process with AR coefficient
    $\phi \in \{-0.4, 0.2, 0, 0.2, 0.4\}$ and series length
    $n \in \{100, 200, 400, 800, 1600, 3200\}$ transformed to
    an exponential distribution with $\lambda = 1$ based on 10,000 replicates. 
...
\begin{figure}[tbp]
  \centering
  \includegraphics[width=\textwidth]{figures/plot_exp_rho}
  \caption{Empirical coverage rates of different 95\% block bootstrap CIs for 
    the first-order autocorrelation coefficient $\rho$ of an AR(1) process with
    $\phi \in \{-0.4, 0.2, 0, 0.2, 0.4\}$ and series length
    $n \in \{100, 200, 400, 800, 1600, 3200\}$ transformed to
    an exponential distribution with $\lambda = 1$ based on 10,000 replicates. 
    The error bars represent 95\% CIs of the real coverage rates.}
...
Additionally, another set of 50 CIs was generated for each of the percentile, 
BC, and recentered percentile approaches for exponentially distributed
samples of $n \in \{800, 1600\}$.

\begin{figure}[tbp]
  \centering
  \includegraphics[width=\textwidth]{figures/exp_rho_intervals}
  \caption{50 replicate percentile, BC, and recentered percentile CIs for 
    samples of $n \in \{800, 1600\}$ for an AR(1) process with $\phi = 0.4$ 
    transformed to an exponential distribution iwth $\lambda = 1$. For 
    each replicate, the lower and upper bounds of the CIs are displayed, as well 
    as $\hat\theta_n$ (blue circle) and $\bar\theta_n^{B}$ (red cross).}
  \label{fig:eri}
\end{figure}

Although there are differences in the ranges of sample sizes needed to cover 
parameters of a normally distributed AR(1) process versus an exponentially 
distributed AR(1) process, the trends and patterns are generally the same.
Student's $t$ CIs still appear to yield higher coverage rates than other 
methods. Performance is still generally better when temporal dependence is 
negative rather than positive. Most importantly, percentile, BC, and BCA CIs
still deteriorate in coverage of $\rho$ as sample size increases. As exhibited 
in \textbf{Figure~\ref{fig:eri}}, percentile and BC CIs still have the same bias
issue.
