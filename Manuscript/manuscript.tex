...
Let \jy{No need to number an equation if it is not referred to}
\mc{addressed}
\begin{equation*}
...
\jy{I like the font size of the labels and axis of Figure 4. They are still
  overly large in Figures 1--3. The panel head is also too wide. The axis ticks
  look crowded. The panel
  labels on the right also does not look pretty. I'd prefer the old version if
  the font sizes were smaller.}
\mc{addressed}
...
\jy{Is textbf of Figure requried? Never manual formatting.}
\mc{The guide says "When you need to refer to Figure 1, Reference 1, Table 1, 
Scheme 1, Step 1, and Equation 1 please bold those words in the text (size 10) 
and in the title (size eight)." If I don't use textbf, the references don't get
bolded by default.}
...
AR(1) process with an underlying unit exponential marginal distribution. The
...
\citep{nelsen2006introduction} Then we computed $E[XY]$, which 
...
\jy{Let $\rho$ and $\phi$ be the autocorrelation coefficient of $X_t$ and $W_t$,
  respectively. There is a 1-1 map between $\rho$ to $\phi$. You already figured
  out how to transform from $\rho$ to $\phi$. Now we want $\phi$ to be, say,
  0.4, you can calibrate $\rho$ to give you the desired value. This only need to
  be done once before your run your simulation. }

\jy{cite the copula book instead of the package} \mc{i cited the nelsen book 
referenced in the manual}

\jy{comment on the similarity of the results with those reported in the main 
text.} \mc{addressed}
...
Iit seems that range of sample sizes needed 
to cover parameters of an exponentially distributed AR(1) process is generally 
higher than that needed to cover parameters of a normally.
distributed AR(1) process. Despite this, the trends and patterns discussed 
previously with respect
to the performance of different methods and with different parameters are the 
same.
For instance, Student's $t$ CIs still appear to yield higher coverage rates than 
other 
...
in Figure~\ref{fig:eri}, percentile and BC CIs still have the same bias
issue. Overall, the results with an underlying marginal exponential distribution 
are very similar to the results with an underlying normal distribution from the
main study. 
