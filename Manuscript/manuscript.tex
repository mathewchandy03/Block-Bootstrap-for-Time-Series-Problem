...
distribution. The true mean and standard deviation parameters are~1. The lag-1
autocorrelation coefficient is not invariant to the
transformation\citep{hofert2018ealements}, but its value can be obtained by
\begin{equation*}
...
distribution with correlation~$\phi$. We kept the configuration of
$\phi \in \{-0.4, -0.2, 0.0, 0.2, 0.4\}$, and the corresponding lag-1
autocorrelation coefficients are $\{-0.3xx, ... ... \}$.
...
\begin{figure}[tbp]
  ...
  \caption{50 replicate percentile, BC, and recentered percentile CIs for the
    mean, standard deviation, and lag-1 autocorrelation coefficient
...
\jy{change the captions of the other three figures accordingly}

The empirical coverage rates for $\mu$, $\sigma_w$, and $\phi$, as well as 95\%
...
samples of $n \in \{800, 1600\}$ in Figure~\ref{fig:eri}.
...
confidence interval continues to be effective in estimating $\phi$ due to the
