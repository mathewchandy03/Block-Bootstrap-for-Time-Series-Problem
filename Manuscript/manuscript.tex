...
size $l$ is constructed from the original sample. The set would take the form:
$\{[X_1, ..., X_l], [X_{l+1}, ..., X_{2l}], ..., [X_{n-l+1}, X_n]\}$. Each
bootstrapped pseudo-sample is created by sampling $n / l$ blocks with
replacement from the same set.
...
$i <= n$, meaning that $n - l + 1$ blocks are available to resample from. This
set takes the form:
$\{[X_1, ..., X_l], [X_2, ..., X_{l+1}], ..., [X_{n-l+1}, X_n]\}$. 
The set can also include blocks that ``wrap around'': in
other words, blocks that end with a lesser index than the index they began
with, meaning that $n$ blocks are available to resample from. This set takes
the form:
$\{[X_1, ..., X_l], [X_2, ..., X_{l+1}], ..., [X_n, X_{l-1}]\}$.
If $l \vert n$,
...
The block should be of size $l$ large enough for each
...
\jy{Paragraph: basic percentiles; use the terminologies in the literature.}

In this study, 1000 $\hat\theta_n^{(b)}$ were created for one simulation of
block bootstrap estimation. Using this distribution of $\hat\theta_n^{(b)}$,
several variants of a $1 - \alpha$/2 \% confidence interval for the parameter
can be constructed. The most simplistic method for constructing this interval
is the basic percentile confidence interval. For a parameter $\theta$, the
percentile $1 - \alpha$/2 \% confidence interval takes the form:
...
\begin{figure}[tbp]
  ...
  \caption{Plots of $\mu$ coverage rates for each interval type and $\phi$
    ...
    rate. The error bars represent a 95\% confidence interval for the coverage
