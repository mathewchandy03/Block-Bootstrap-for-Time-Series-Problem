...
\author{Mathew Chandy*, Elizabeth D. Schifano, \& Jun Yan \\ \medskip \medskip 
Department of Statistics, University of Connecticut, Storrs, CT \\ \medskip\medskip 
https://doi.org/10.33697/ajur.2024.101\\ \medskip\medskip 
...
\section*{\textit{Block Bootstrap CIs}}
\label{sec:bbci}
...
\begin{figure}[tbp]
  ...
  Top: $l = \lceil n^{1/3} \rceil$. Bottom: $l = \lceil 2n^{1/3} \rceil$.}
  \label{fig:norm1}
\end{figure}


For estimating the mean parameter~$\mu$, the top panel of
\textbf{Figure~\ref{fig:norm1}} suggests 
...
The results for $l = \lceil 2n^{1/3} \rceil$ are reported in the
bottom panel of Figure~\ref{fig:norm1}. The performances generally
seems to be inferior compared those with $l = \lceil n^{1/3}\rceil$, but
importantly, the patterns in performance when varying other parameters
appear to be robust to the different block size.
For negative autocorrelations, the coverage 
...
with $l  \in \{\lceil n^{1/3} \rceil, \lceil 2n^{1/3} \rceil$, as well as 95\%
...
negative rather than positive. Again, altering the block size results
in the same changes in performance of different CIs as those in the
scenario of  marginal normal distribution.
Of particular significance, the percentile, BC,
...
\bibliographystyle{ajur}
\input{citation.txt}


\section*{About Student Author}
\label{sec:asa}
Mathew Chandy is a senior majoring in both Statistics and Statistical Data Science, 
and he plans to graduate in the Spring of 2024.


\section*{Press Summary}
\label{sec:press}
This simulation study evaluates the sample size necessary to estimate the mean,
standard deviation, and lag-1 autocorrelation of a stationary time series using
different block bootstrap confidence interval types. The results showed that 
percentile-based confidence intervals for the lag-1 autocorrelation may suffer 
from coverage deterioration as 
sample size is increased, motivating the authors to propose a new recentered percentile 
confidence interval which does not deteriorate in performance for greater sample sizes.
The results also suggest that when using Student's $t$ bootstrap confidence intervals, 
a sample size of at least 1600 may be sufficient to estimate the mean, 
whereas a sample size larger than 3200 may be necessary to estimate 
the standard deviation. The results additionally indicate that 
estimation of the lag-1 autocorrelation - using
Student's $t$ bootstrap confidence intervals - demands a sample size of at least 100 
when the marginal distribution is standard normal and a sample size of at least 1600 
when the marginal distribution is unit exponential.
