...
a Student's $t$ CI, a percentile CI, a bias-corrected CI, a bias-correct and
...
increases. Student's $t$ CIs generally does not need relatively large sample
sizes to achieve correct coverage, except for samples with strong negative
...
the coverage intervals for other methods contain 95\%. In fact, Standard,
Percentile, $BC$, and $BC_a$, and Proposed Method CIs require similar sample
sizes to to recover $\mu$ at the nominal level for all combinations of $n$ and
...
For example, $BC$ CIs recovers $\mu$ for $n \geq 100$ when $\phi = -0.2$, but
...
Figure~\ref{fig:sigma} suggests that each method can reach nominal coverage of
...
To summarize, the CI type chosen should be based on the target parameter. When
...
the temporal dependence increases, one must know what the parameter of
interest is, and in the case of $\mu$: $\phi$'s sign.
...
\begin{figure}[tbp]
  ...
  \caption{100 replicate Percentile, Bias-Corrected, and Proposed Method
    ...
    are displayed, as well as $\hat\theta_n$ (blue) and $\bar\theta_n^{(b)}$
    (red). }
  \label{fig:npi}
\end{figure}
...
address this question, plots of 100 CIs using each of the Percentile, $BC$,
and Proposed methods were constructed. As shown in Figure~\ref{fig:npi},
Percentile CIs appear to suffer from bias; specifically, they seem to heavily
underestimate $\phi$. $BC$ CIs do seem to correct this bias on average, but
the width of the CIs is very inconsistent. Our proposed CIs correct the bias
using $\widehat{\text{bias}} = \bar\theta_n^{(b)} -  \hat\theta_n$, which
seems to usually be negative, indicating a positive correction from interval
construction in Section~\ref{sec:bbci}.
...
\jy{Keep the tex source tidy. Clean it up.}\mc{addressed}
...
Again, this analysis relies on the assumption that there is a size $n$ large
...
\eds{are there other methods that you know of?} \mc{mentioned an alternative
  to block bootstrap}
