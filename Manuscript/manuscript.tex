...
block, and let $a$ be $SKEW\{\hat{\theta}_{n, -i}\} / 6$. Then
 $\hat\theta_{BCA}[\alpha] = \hat{G}^{-1}\Phi(z_0 + \frac{z_{0} + z_{\alpha}}{1 - a(z_{0} + z_{\alpha})}).$ \eds{notation will need to be cleaned and clarified}
A $(1 - \alpha)$ centered percentile \eds{BCA?} CI is
\[
...
$\hat\theta_{L, CI}$ and $\hat\theta_{U, CI}$ be the lower and upper bound,
respectively,  for $CI \in \{1, \ldots, n_{CI}\}$, where $n_{CI}$ is the number 
of CIs.
...
Based on Figure~\ref{fig:mu}, the performance is
about the same for the three interval types studied for $\mu$. As dependence 
gets stronger,
a larger sample size is necessary. For $\mu$, all three types are working asymptotically 
but for finite sizes, they more or less need about the same sample size to have 
the correct coverage. When $\phi < 0$,
$n = 100$ seems to  be large enough to cover $\mu$ at the nominal rate. When
$\phi = 0$, or in other other words, the sample is independently normally
distributed, %from a normal distribution,
$n = 500$ \eds{$n=400$?} seems to be large enough to cover $\mu$ at the nominal 
rate. Samples
...
used when the target parameter is $\mu$. \eds{I would argue that none of the 
sample sizes considered work well for $\phi=0.2$ or $\phi=0.4$ based on 
Figure~\ref{fig:mu} and your criteria above (``If all values in the interval are
 below 0.95, the results would suggest that the
method either is providing inaccurate estimation, is underestimating the
process' variability, or a combination of both.")}


Based on Figure~\ref{fig:sigma}, coverage is
notably lower for $BC_a$ intervals \eds{use consistent notation for this}, 
but $\hat{\theta}_{n}$-centered intervals
and basic intervals have similar performance. For these two interval types,
$n = 500$ seems to be necessary to cover $\sigma$ 
\eds{use $\sigma_x$ instead of $\sigma$} at the nominal rate when
...
process has a greater $\phi$ of $0.4$, $n = 400$ may be necessary. 
\eds{$n=400$ does not appear to be enough to me. I would also argue larger $n$
is needed for the other values of $\phi$ unless the curves asymptote at a value
lower than 0.95. Do the point estimates eventually get closer to the nominal 
level with larger $n$?}  
It is more
...
nominal rate as $n$ increases appears to be invalid based on 
Figure~\ref{fig:sigma}.
\eds{Figure~\ref{fig:phi}?}
The $BC_a$ interval performs very poorly and deteriorates more noticeably
when the absolute value $\phi$ gets
higher. Even when $\phi = 0$, $\phi$ is still undercovered for $n$ as high
...
over-cover $\phi$. Regardless of whether $\phi$ is positive or negative, these 
two intervals do not perform well. These results suggest that $BC_a$ and basic 
percentile
...
intervals seem to recover $\phi$ correctly as long as $n \geq 200$, and this
...
\eds{not sure if above paragraph is needed - seems to repeat information in the 
...
We know theoretically that the block bootstrap procedure will
perfectly estimate \eds{? interval will correctly cover?  What does the theory 
actually say?} a parameter of a time series given an infinitely large
sample, so the goal for this study was to find the smallest finite
sample length $n$ of a time series in order for the block bootstrap procedure to 
recover its associated parameters at an acceptable rate.

Again, this \eds{what?} relies on the assumption that
...
construction that would more appropriately recover the parameters of a time
