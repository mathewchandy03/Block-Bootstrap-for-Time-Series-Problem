...
\jy{not an AR(1) process...} \mc{addressed}
parameters of a marginally transformed AR(1) process with an underlying 
exponential distribution is discussed in the Appendix. 
...
\begin{figure}[tbp]
  ...
  \caption{Empirical coverage rates of different 95\% block bootstrap CIs for
    the marginal mean $\mu$ of a marginally transformed AR(1) process with AR 
    coefficient
    $\phi \in \{-0.4, 0.2, 0, 0.2, 0.4\}$ with an 
    underlying exponential distribution ($\lambda = 1$) and series length
...
\begin{figure}[tbp]
  ...
  \caption{Empirical coverage rates of different 95\% block bootstrap CIs for
    ...
    process with AR 
    coefficient $\phi \in \{-0.4, 0.2, 0, 0.2, 0.4\}$ with an 
    underlying exponential distribution ($\lambda = 1$) and series length 
...
\begin{figure}[tbp]
  ...
  \caption{50 replicate percentile, BC, and recentered percentile CIs for 
    samples of $n \in \{800, 1600\}$ for a marginally transformed AR(1) process 
    with $\phi = 0.4$
    \jy{not accurate} \mc{is this better}
    with an underlying exponential distribution ($\lambda = 1$). For 
    each replicate, the lower and upper bounds of the CIs are displayed, as well 
    as $\hat\theta_n$ (blue circle) and $\bar\theta_n^{B}$ (red cross).}
  \label{fig:eri}
\end{figure}
...
in Figure~\ref{fig:eri}, percentile and BC CIs still suffer from the same bias
issue, whereas recentered percentile CI still work when estimating $\phi$ 
because the 
original point estimator is unbiased. Overall, the results with an underlying 
marginal 
...
\jy{comment on that the recentered percentile CI for autocorrelation still works
  as the point estimator is unbiased.} \mc{addressed}
