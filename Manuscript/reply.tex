...
\jy{Be selective here for ``notable''. I'd not include items like `wording
  clarification'; your could put all the clarifications as one
  item. Keep this list short for the reviewers to have a  
  quick idea about the major changes (e.g., additional simulation studies).}
\mc{addressed}

\begin{enumerate}
\item Added clarifications of design and more detail examination of results;
...
biased estimator of the target while the spreadness of the CIs still
provide good uncertainty measures. This has now been added in the
Recentered Percentile CI pargarph.
...
\jy{put the legend on one line.}
\mc{addressed}

\jy{This plot seems fine to me. Could you make it for the other
  parameters too? Then we can make a decision. If we use this, the
  number of figures would be greatly reduced.}
\mc{addressed}
...
\begin{figure}[tbp]
  \centering
  \includegraphics[width=\textwidth]{figures/alt_plot_norm_sigma_1}
  \caption{Empirical coverage rates of different 95\% block bootstrap CIs for
    the marginal mean $\sigma$ of an AR(1) process with a marginal standard 
    normal distribution with AR coefficient
    $\phi \in \{-0.4, 0.2, 0, 0.2, 0.4\}$ and series length
    $n \in \{100, 200, 400, 800, 1600, 3200\}$ based on 10,000 replicates. The
    error bars represent 95\% CIs of the real coverage rates.}
...
\begin{figure}[tbp]
  \centering
  \includegraphics[width=\textwidth]{figures/alt_plot_norm_phi_1}
  \caption{Empirical coverage rates of different 95\% block bootstrap CIs for
    the marginal mean $\phi$ of an AR(1) process with a marginal standard 
    normal distribution with AR coefficient
    $\phi \in \{-0.4, 0.2, 0, 0.2, 0.4\}$ and series length
    $n \in \{100, 200, 400, 800, 1600, 3200\}$ based on 10,000 replicates. The
    error bars represent 95\% CIs of the real coverage rates.}
...
\jy{Need to explain ``table'' methods better in the manuscript.}
\mc{I gave it a try}
...
\jy{This is still not clear. I think they mean CIs based on asymptotic
  distributions with estimated asymptotic variances. This is in
  contrast to percentile based CIs.}
\mc{I tried to make it more clear in the manuscript, I used ``percentile-based"
in the manuscript instead of CIs that are not based on tables}
...
Figure~\ref{fig:alt_phi1} is provided as an example:
...
section. We have also added a comment in the discussion regarding what would
be expected for higher dependence levels. \jy{Add a comment in the discussion about what'd be expected
  with higher dependence levels.}
